Dataset: TCGA Colorectal Cancer (COAD)
Source: UCSC Xena / cBioPortal
Samples: 478
Description: Contains genomic mutation, stage, and survival information.
...
Conversational Setup (GUI): A user interacts with the system via a Graphical User Interface (GUI), submitting natural language queries.

Logic Extraction (The "Brain"): A local LLM (Llama3) acts as a reasoning agent. It does not analyze the data directly; instead, it interprets user instructions and converts them into executable code and logic rules (e.g., parsing "Age > 50" into a filter operation).

Automated Analysis (The Engine): The system executes the generated logic against local data files to perform prevalence testing, association analysis, and survival modeling.
...
Bash
git clone https://github.com/your-username/ai-hope-implementation.git
